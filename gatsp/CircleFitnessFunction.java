 ...
 */
public class CircleFitnessFunction implements FitnessFunction{
    ...
    /** Creates a CircleFitnessFunction with a random seed. */
    public CircleFitnessFunction(Landscape l, int width, int height){
	this.l = l;
	build_polar_coordinates(width, height);
    }

    public void build_polar_coordinates(int width, int height){
	...
	// calcuate worst r * number of cities	
	//worst r is centre to corner over and over, since rodner is
	worst_case = Math.sqrt( (width * width) + (height * height) ) * l.size();
	
    }



    //worst case is centre to one corner n times, or
